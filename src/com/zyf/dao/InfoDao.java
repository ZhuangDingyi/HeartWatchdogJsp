...
public class InfoDao {
    ...
    public List queryRelaInfo(int uid,int reid) {
        String sql = "";
        sql = "select * from info where uid="+reid+" and uid in (select reid from userrela where uid="+uid+" and status=1) union select * from info where uid="+reid+" and uid in (select uid from userrela where reid="+uid+" and status=1)";
        ArrayList list = null;
        Info info = null;
        ResultSet rs = this.connection.executeQuery(sql);
        if (rs != null) {
